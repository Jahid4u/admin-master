...
export const Route = createFileRoute("/admin/site/privacy")({ component: PrivacyAdminPage });
...
const defaults: Privacy = {
  ...
  sections: [
    { heading: "1. Information We Collect", body: "When you contact me through the website or email, I collect the information you choose to share — typically your name, email address, and the contents of your message. No tracking cookies or analytics scripts are used to profile you personally.", enabled: true },
    { heading: "2. How We Use It", body: "Your information is used solely to respond to your inquiry, discuss potential collaboration, and deliver the work we agree upon. I do not sell, rent, or share your data with third parties for marketing.", enabled: true },
    { heading: "3. Data Retention", body: "Project communications are kept for as long as our working relationship is active and for a reasonable period afterwards for reference. You can request deletion at any time.", enabled: true },
    { heading: "4. Security", body: "Reasonable technical measures are in place to protect your data, but no method of transmission over the internet is 100% secure.", enabled: true },
    { heading: "5. Contact", body: "For any privacy-related questions, reach out at jahidmail2020@gmail.com.", enabled: true },
  ],
};
...
function PrivacyAdminPage() {
  ...
  const initial = { ...defaults, ...((data?.privacy_page as object | undefined) ?? {}) } as Privacy;
  const [v, setV] = useState<Privacy>(initial);
  ...
  const set = (patch: Partial<Privacy>) => setV({ ...v, ...patch });

  const saveMut = useMutation({
    mutationFn: () => update({ data: { key: "privacy_page", value: v } }),
    onSuccess: () => {
      toast.success("Saved");
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); saveMut.mutate(); }} className="max-w-2xl">
      <PageHeader
        ...
        description="Edit every block of the /privacy page in its own container."
        actions={
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          ...
          <SectionCard
            title="Legal badge"
            description="Small pill with the shield icon above the title."
          ...
          <SectionCard
            title="Sections"
            description="Each numbered policy section with its heading and body. Add, remove, reorder by editing."
          >
            {v.sections.map((s, i) => (
